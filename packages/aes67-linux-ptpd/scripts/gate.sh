...
if [ "${AES67_ANALYSE:-0}" = "1" ] || [ "${analyse:-0}" = "1" ]; then
  ...
  if [ -n "$cppcheck_bin" ]; then
    ...
    "$cppcheck_bin" --quiet --error-exitcode=1 --enable=warning,performance,portability \
      ...
      -I src -I ../aes67-core -I ../aes67-profiles -I ../t41-ptp/src src/ || { echo "FAIL: cppcheck" >&2; exit 1; }
  else
  ...
  build-san/FuzzPtpWire "${AES67_FUZZ_ITER:-200000}" || { echo "FAIL: fuzzing" >&2; exit 1; }
  ...
  ( cd build-cov && find . -name '*.gcda' -print0 | xargs -0 $gcov_tool -p > /dev/null 2>&1; \
    ls *.gcov 2>/dev/null | grep '#packages#aes67-linux-ptpd#' | grep -vE '#Tests#|#external#|#doctest#|#build' \
    | LC_ALL=C xargs -r awk -F: '/^ *#####:/ { miss++ } /^ *[0-9]+[*]?:/ { hit++ } \
      END { t = hit + miss; if (t == 0) { print "coverage: nothing measured"; exit 0 } \
            printf "coverage: %d/%d lines of aes67-linux-ptpd, %.1f%%\n", hit, t, 100 * hit / t }' )
  rm -f build-cov/*.gcov
else
