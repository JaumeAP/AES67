...
Test and interop tooling, the whole inventory. Everything in `packages/aes67-macos-driver/Tools/` except item 8, and everything builds in the gate; run the simulations after touching SDP generation, SAP or the profiles.

1. `AES67InteropSim` — offline interop against the RAVENNA Linux daemon. The daemon's own send and receive path is `Tests/support/DaemonSap.{h,cpp}`, mirrored from `daemon/sap.cpp`, and the tool reports on that oracle instead of restating the daemon's rules. Sections: its SAP announcement through our `SAPListener`, its SDP through our parser, the flow under every profile, our announcement through the daemon's receive path, and our SDP against the shape it announces.
2. `DanteInteropSim` — the same shape against Dante Controller 4.18.1.1, both directions, every profile, with what only a live Controller can settle counted separately.
3. `AES67LiveDaemonInterop` — the live counterpart of item 1, against a real `aes67-linux-daemon` built with `-DFAKE_DRIVER=ON` and started by a Linux CI job.
...
9. All eleven tools above were walked through on 2026-09-15/16, one at a time, on branch `tool/interop-sim-rewrite`. What each one was found doing, and what changed: items 1, 2 and 3 built their transmit session by hand, so a change to what the driver announces never reached them -- `NetworkEngine/TxSession.h` is now the one place those fields are decided, called by `createTxStream()` and by all three; item 1 was rewritten besides, because two of its four sections could not fail (our codec against itself, and a SAP header it wrote and read back); item 2 printed a false claim, that our SAP packets omit the `application/sdp` payload type; item 4 hand-rolled that same header without the payload type, so the daemon could not hear it, never withdrew its session and started every run at sequence 0; item 5 validated `--channels` and never checked it; item 6 printed a deletion's empty name instead of the identity that says what it withdraws; item 7 counted a packet as reordered before the clamp that kept it in order; item 8 printed an RTSP URL with a raw space in it; item 10 had no CMake target at all, which is how it kept two variable-length arrays nobody compiled; item 11 wrote its samples into the same stdout its engines log to. Items 9 and 11 were found sound -- item 9 was checked against a third-party HAL device, since this Mac has no AES67 driver installed (open item 1), and item 10 was built but not run for the same reason.
