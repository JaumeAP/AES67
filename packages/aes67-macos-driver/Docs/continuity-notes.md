...
9. The eleven tools listed above are being walked through one at a time, checking that each still does what it claims. Done: item 1, rewritten -- it used to check an L24 payload encoded and decoded by our own codec, and a SAP header it wrote into a buffer and read back, neither of which could fail whatever the daemon did. Items 9 and 10 need the driver installed, which this Mac does not have (open item 1).
