...
9. The eleven tools listed above are being walked through one at a time, checking that each still does what it claims. Done: items 1, 2 and 3. Item 1 was rewritten -- it used to check an L24 payload encoded and decoded by our own codec, and a SAP header it wrote into a buffer and read back, neither of which could fail whatever the daemon did. All three built their transmit session by hand, so a change to what the driver announces never reached them; `NetworkEngine/TxSession.h` is now the one place those fields are decided, called by `createTxStream()` and by the three tools. Item 2 also printed a false claim, that our SAP packets omit the `application/sdp` payload type. Items 9 and 10 need the driver installed, which this Mac does not have (open item 1).
