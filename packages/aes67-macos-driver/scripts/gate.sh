...
#
# Usage:
#   scripts/gate.sh            incremental build in build/
#   scripts/gate.sh --clean    wipe build/ first, like a fresh runner
#
# This package's gate. The monorepo's scripts/gate.sh runs it along with
# every other package's, and .githooks/pre-push runs that. Set SKIP_LOCAL_CI=1, or push
...
#
# AES67_ANALYSE=1 turns it on; scripts/gate.sh --analyse does the same.
