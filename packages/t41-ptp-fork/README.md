...
- **The branch it did live on has been rewritten since.** The 2022 commit this fork is built on is no longer served by upstream at all: asking for it answers `couldn't find remote ref`. It survives on HedgeHawk's `ieee1588-2-fix`, which is what the copy here is made of.
...
`applyProfile()` sets the whole combination a given ecosystem expects in one call, from a `PTPBase::ProfileSettings`: a domain, a `majorSdoId` and the three log2-second intervals. It holds no table of its own, and it does not read anyone else's: this library compiles with nothing beside it. `examples/PTPNode` writes the five numbers of the AES67 media profile out -- domain 0, `majorSdoId` 0, `-3`, `0`, `-3`. A sketch built inside the `JaumeAP/AES67` monorepo can read the same five, and the 1588-2008 default and 802.1AS besides, from `packages/aes67-profiles/Profiles/PtpProfiles.h`, which the macOS core reads too. It sets the numbers only: the delay mechanism is chosen when the object is built, and the transport by which class is used, `l3PTP` for AES67 and `l2PTP` for 802.1AS. The individual setters remain, for anything a profile does not cover.
