...
`applyProfile()` sets the whole combination a given ecosystem expects in one call, from a `PTPBase::ProfileSettings`: a domain, a `majorSdoId` and the three log2-second intervals. It holds no table of its own. The three combinations that matter -- the IEEE 1588-2008 default, the media profile AES67 and RAVENNA gear runs, and 802.1AS -- are data in `packages/aes67-profiles/Profiles/PtpProfiles.h`, freestanding headers this library's board build has on its include path and the AES67 macOS core reads as well, so the two sides agree by construction instead of by two copies. A sketch that has only this library writes the five numbers itself. It sets the numbers only: the delay mechanism is chosen when the object is built, and the transport by which class is used, `l3PTP` for AES67 and `l2PTP` for 802.1AS. The individual setters remain, for anything a profile does not cover.
