...
Platform-free core: `JaumeAP/aes67-core`, consumed as the `external/aes67-core` submodule — SDP, RTP wire header, jitter buffer, packet pool, PLL, resampling, channel mapping, configuration, the ring buffer, the RT-safe view over it, the PTP time types and clock-source interface, and the PTP peer, RTCP receiver and Dolby model tables. What stays here is macOS plus `aes67_net`, the socket layer, which supplies the four `NetworkUtils` symbols the core declares without implementing. The boundary was reviewed on 2026-09-03 and deliberately left where it is. Clone with `--recurse-submodules`.
