...
| [`packages/aes67-ravenna`](packages/aes67-ravenna) | RAVENNA's session layer: DNS-SD over mDNS so a device finds what this machine offers, and the RTSP DESCRIBE that hands over the SDP. The SDP is the core's and the channel routing is the core's matrix; what is here is discovery and description | CMake, C++20 |
