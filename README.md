...
| [`packages/aes67-ravenna`](packages/aes67-ravenna) | RAVENNA's session layer and NMOS IS-05: DNS-SD over mDNS so a device finds what this machine offers, the RTSP DESCRIBE that hands over the SDP, and the connection API that assigns one device's stream to another's channels. The SDP is the core's and the routing is the core's matrix | CMake, C++20 |
