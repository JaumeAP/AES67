...
| [`packages/aes67-ravenna`](packages/aes67-ravenna) | RAVENNA's session layer and the NMOS APIs that route it: DNS-SD over mDNS and the RTSP DESCRIBE that hands over the SDP, IS-05 to give one device another's stream, and IS-08 for the grid, channel by channel. The SDP is the core's and the grid is the core's matrix | CMake, C++20 |
