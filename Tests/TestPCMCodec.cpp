...
#define CHECK(cond, msg) \
    if (!(cond)) { std::cerr << "FAIL: " << msg << std::endl; testsFailed++; return false; } \
    else { testsPassed++; }
...
bool testL16KnownVectors() {
    ...
    for (size_t i = 0; i < in.size(); ++i) {
        ...
        CHECK(out[i * 2] == hi && out[i * 2 + 1] == lo,
              "L16 byte mismatch at sample " + std::to_string(i));
    }
    // Full-scale positive clamps to 0x7FFF; full-scale negative to -32767 = 0x8001.
    CHECK(out[2] == 0x7F && out[3] == 0xFF, "L16 +1.0 must be 0x7FFF");
    CHECK(out[4] == 0x80 && out[5] == 0x01, "L16 -1.0 must be 0x8001");
    std::cout << "PASS" << std::endl;
    return true;
}

bool testL16RoundTrip() {
    ...
    for (size_t i = 0; i < in.size(); ++i) {
        ...
        CHECK(std::fabs(out[i] - in[i]) < 1.0e-4f,
              "L16 round-trip drift at " + std::to_string(in[i]) + " -> " + std::to_string(out[i]));
    }
    std::cout << "PASS" << std::endl;
    return true;
}

bool testL24KnownVectors() {
    ...
    // 0.0 -> 0x000000
    CHECK(out[0] == 0 && out[1] == 0 && out[2] == 0, "L24 0.0 must be 0x000000");
    // +1.0 -> 8388607 = 0x7FFFFF
    CHECK(out[3] == 0x7F && out[4] == 0xFF && out[5] == 0xFF, "L24 +1.0 must be 0x7FFFFF");
    // -1.0 -> -8388607 = 0x800001
    CHECK(out[6] == 0x80 && out[7] == 0x00 && out[8] == 0x01, "L24 -1.0 must be 0x800001");
    std::cout << "PASS" << std::endl;
    return true;
}

bool testL24RoundTrip() {
    ...
    for (size_t i = 0; i < in.size(); ++i) {
        CHECK(std::fabs(out[i] - in[i]) < 1.0e-6f,
              "L24 round-trip drift at " + std::to_string(in[i]));
    }
    std::cout << "PASS" << std::endl;
    return true;
}

bool testDecodeKnownBytes() {
    ...
    CHECK(std::fabs(f16 - 0.5f) < 1e-6f, "0x4000 must decode to 0.5");
    // 0x400000 = 4194304 -> /8388608 = 0.5
    uint8_t l24[3] = {0x40, 0x00, 0x00};
    float f24 = 0;
    decodeL24BE(l24, 1, &f24);
    CHECK(std::fabs(f24 - 0.5f) < 1e-6f, "0x400000 must decode to 0.5");
    std::cout << "PASS" << std::endl;
    return true;
}

bool testInterleaveRoundTrip() {
...
int main() {
    std::cout << std::endl << "PCM Codec Tests" << std::endl << std::endl;
    testL16KnownVectors();
    testL16RoundTrip();
    testL24KnownVectors();
    testL24RoundTrip();
    testDecodeKnownBytes();
    testInterleaveRoundTrip();
    std::cout << std::endl << "Passed: " << testsPassed << ", Failed: " << testsFailed << std::endl;
