...
generic skills — `git-rules`, `changelog-rules`, `file-operations`,
...
2. **Import** (a config bundle uploaded into a session — "reverse
   ...
   2.3. **Contradiction check, mandatory on every import**: every
        ...
        rules elsewhere
        (`git-rules`' merge-policy clause, `CLAUDE.md`'s HANDOFF.md
        close-regeneration clause) also state one at a time. After
        ...
        different merge policy, different HANDOFF.md handling, a
        ...
        "(superseded by an imported rule, YYYY-MM-DD — see git-rules'
        merge policy)") — don't delete or rewrite the target's original
   ...
   2.4. **`CLAUDE.md`, common part only**: same substitution treatment
        ...
        substituted. This includes the "Session continuity — HANDOFF.md"
        section: any old rules about `HANDOFF.md` already in the target
        repo's `CLAUDE.md` get deleted along with the rest of the common
        part, not kept or patched in alongside the incoming version.
   ...
   2.5. **The four mandatory generic skills, always the incoming
        version**: `git-rules`, `changelog-rules`, `file-operations`,
        ...
        match). Same substitution treatment as 2.2/2.4 — these four
   ...
   2.7. **Recommended-but-not-bundled skills, always offer too**: once
        ...
        **Batch the git side, don't sync per pick** (2026-07-24): each
        install inside the loop still gets its own commit (git-rules
        rule 4, one commit per unit of work), but hold off on push,
