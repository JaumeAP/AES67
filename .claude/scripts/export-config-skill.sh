...
# API upload path rejects more than one). This repo's mandatory bundle
# has FOUR skills (git-rules, changelog-rules, file-operations,
# find-skills), each with its own SKILL.md -- those get renamed to
...
description: Portable Claude Code .claude/ configuration bundle, common CLAUDE.md rules, settings.json, every hook, the git-rules/changelog-rules/file-operations/find-skills skills, and the full export/import procedure, all packaged as one file. Use this skill whenever asked to export or import Claude Code configuration between repos, whenever the user says "exporta la configuració"/"importa la configuració" or the English equivalent, or when setting up a brand new repo's .claude/ tooling from an existing one. Also use it when a config bundle is pasted or uploaded with no explicit request, to recognize it and know how to apply it.
---
...
- `references/recommended-skills.txt` -- optional skill packs a target
  repo can offer to fetch live via `find-skills`, not bundled as files.
...
- `references/skills/<name>/<name>.md` -- the four mandatory generic
  skills (`git-rules`, `changelog-rules`, `file-operations`,
...
# Trim skills-lock.json to just find-skills' own entry, same as the old
# export-config.sh used to -- the full file also tracks whatever else
# this repo happens to have installed right now, none of which is in
# this bundle.
if [ -f skills-lock.json ]; then
  jq '.skills |= {"find-skills": .["find-skills"]}' skills-lock.json > "$skill_dir/references/skills-lock.json"
fi

for hook in check-handoff-pushed.sh check-reply-format.sh \
  close-command-reminder.sh config-ingest-reminder.sh \
  init-submodules.sh local-merge-reminder.sh no-commit-on-main.sh \
  precompact-hygiene-flag.sh require-handoff-read.sh \
  rule-check-reminder.sh skill-creation-reminder.sh \
  sync-command-reminder.sh; do
  [ -f ".claude/hooks/$hook" ] && cp ".claude/hooks/$hook" "$skill_dir/references/hooks/"
done

for skill in git-rules changelog-rules file-operations find-skills; do
