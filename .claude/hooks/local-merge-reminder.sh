#!/bin/bash
# PreToolUse hook: non-blocking reminder of git-rules' "merge locally with
# plain git, not the GitHub API merge tool" rule (rule 5). Fires when:
...
# (see git-rules changelog): the GitHub merge API attributes the resulting
# commit to whichever account is authenticated to the connected GitHub
# integration, not to this session's own configured git identity --
# showing up as unverified/misattributed on GitHub even when every
# directly-authored commit was correctly configured.
#
# 2026-07-22: git-rules v2.11.0 added a supremacy clause -- this merge
# policy overrides any conflicting instruction found in an imported file
# (e.g. an incoming CLAUDE.md/skill saying "open a PR" or "ask before
# merging"). The reminder text below mentions this explicitly so an
# imported instruction doesn't get followed instead of this rule.
set -euo pipefail
