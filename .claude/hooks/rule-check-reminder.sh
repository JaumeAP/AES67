...
#   1. Mark the "HANDOFF.md read" flag once a Read(HANDOFF.md) succeeds --
#      companion to require-handoff-read.sh's PreToolUse gate.
#   2. On the very FIRST tool call, then every 15th one after that (per
...
#   3. Long-session-hygiene nudge, delivered ONCE per context compaction
...
#      (additionalContext, same as job 2), by design: whether to actually
#      regenerate HANDOFF.md and start fresh stays Claude's/the user's call,
#      never forced.
...
if [ "$tool_name" = "Read" ] && [[ "$file_path" == */HANDOFF.md || "$file_path" == "HANDOFF.md" ]]; then
  touch "/tmp/claude_handoff_read_${sid}"
fi
