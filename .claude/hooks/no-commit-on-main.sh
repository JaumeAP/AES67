...
# on `main` -- mechanizes git-rules rule 1 ("Never commit directly on
# main"). Added 2026-07-24 after a real incident: a session-close merge
...
jq -n '{hookSpecificOutput: {hookEventName: "PreToolUse", permissionDecision: "deny", permissionDecisionReason: "Bloquejat: estas a punt de fer git commit directament a main, prohibit per git-rules regla 1. Canvia a la branca de treball primer (git checkout <branca>) i fes el commit alla."}}'
