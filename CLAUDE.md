...
**This repository is the base other projects consume as a submodule.** `aes67_core` (platform-free) and `aes67_net` (adds BSD sockets) in `CMakeLists.txt` are the contract; see the README. Nothing reachable from `AES67_CORE_SOURCES` may include an Apple framework or a socket header, transitively — the gate fails on it. When adding a file to the core list, check what its headers pull in, not just the `.cpp`.
