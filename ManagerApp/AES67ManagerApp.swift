...
struct AES67ManagerApp: App {
    ...
    init() {
        ...
        // No install/uninstall tied to launch or quit: DriverManager.init()
        // already calls checkDriverStatus(), so the main window's install
        // switch (ContentView) opens showing whatever's actually there.
        // Installing/removing only happens when the user flips that switch;
        // quitting leaves the driver exactly as the last flip left it.
    }
